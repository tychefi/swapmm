...
template <class T>
T precision_from_decimals(uint8_t decimals)
{
    T p10;
...
asset asset_from_string(string_view from)
{
    ...
    auto precision = precision_from_decimals<safe<int64_t>>(sym.precision());
    amount *= precision;
    amount += fract_part;
...
string asset_to_string(const asset& a) {
    const auto& symbol = a.symbol;
    CHECK(symbol.is_valid(), "Invalid symbol");
   string sign = a.amount < 0 ? "-" : "";
   safe<int64_t> abs_amount = std::abs(a.amount);
   safe<int64_t> precision = precision_from_decimals<safe<int64_t>>(symbol.precision());
   string result = std::to_string( (abs_amount / precision ).value);
   if( precision > 1 )
   {
      auto fract = (abs_amount % precision).value;
      result += "." + std::to_string((precision + fract).value).erase(0,1);
   }
   return sign + result + " " + symbol.code().to_string();
}
